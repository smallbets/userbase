...
const _completeFileUpload = async (uploadChunkParams, itemKey, encryptedFileMetadata, encryptedFileEncryptionKey, database) => {
  uploadChunkParams.itemKey = itemKey
  uploadChunkParams.fileMetadata = encryptedFileMetadata
  uploadChunkParams.fileEncryptionKey = base64.encode(encryptedFileEncryptionKey)

  const action = 'CompleteFileUpload'
  await postTransaction(database, action, uploadChunkParams)
}

const _uploadChunk = async (isFinalChunk, uploadChunkParams, batch, itemKey, encryptedFileMetadata, encryptedFileEncryptionKey, database) => {
  try {
    if (!isFinalChunk) {
      const action = 'UploadFileChunk'

      if (uploadChunkParams.chunkNumber === 0) {
        // server generates file ID on first request. Client will use it for all future requests
        const response = await ws.request(action, uploadChunkParams)
      ...
      await Promise.all(batch)
      await _completeFileUpload(uploadChunkParams, itemKey, encryptedFileMetadata, encryptedFileEncryptionKey, database)
    }

    return uploadChunkParams.fileId
  } catch (e) {
    _parseGenericErrors(e)
...
const _getUploadChunkParams = async (dbId, file, fileId, fileEncryptionKey, position, chunkNumber) => {
  // read a chunk at a time to keep memory overhead low
  const plaintextChunkAsBlob = file.slice(position, position + FILE_CHUNK_SIZE)
  const plaintextChunk = await _readBlob(plaintextChunkAsBlob)
...
const uploadFile = async (params) => {
  try {
    ...
    let fileId // generated by server on first UploadFileChunk request

    while (position < file.size) {
      const uploadChunkParams = await _getUploadChunkParams(dbId, file, fileId, fileEncryptionKey, position, chunkNumber)

      chunkNumber += 1
      position += FILE_CHUNK_SIZE
      const isFinalChunk = position >= file.size

      fileId = await _uploadChunk(isFinalChunk, uploadChunkParams, batch, itemKey, encryptedFileMetadata, encryptedFileEncryptionKey, database)
    }
