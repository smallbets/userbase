...
async function start(express, app, userbaseConfig = {}) {
  try {
    const { httpsKey, httpsCert, stripeRedirectUrl } = userbaseConfig

    if (stripeRedirectUrl)
      process.env["STRIPE_REDIRECT_URL"] = stripeRedirectUrl
    ...
    const httpServer = http
      .createServer(app)
      .listen(httpPort, () =>
        logger.info(`App listening on http port ${httpPort}....`)
      )

    const httpsServer =
      certExists &&
      https
        .createServer({ key: httpsKey, cert: httpsCert }, app)
        .listen(httpsPort, () =>
          logger.info(`App listening on https port ${httpsPort}....`)
        )
    ...
    wss.on("connection", (ws, req, res) => {
      ...
      const sessionId =
        req.query.sessionId && truncateSessionId(req.query.sessionId)

      const conn = connections.register(
        userId,
        ws,
        clientId,
        adminId,
      ...
      if (conn) {
        ...
        const connectionLogObject = {
          ...
          userbaseJsVersion,
          sessionId,
          changePassword: changePassword ? true : undefined,
          route: "Connection",
        }
        const validationMessage = userController.sendConnection(
          connectionLogObject,
          ws,
          res.locals.user,
          res.locals.app
        ...
        ws.on("message", async (msg) => {
          ...
          try {
            logChildObject = {
              ...
              userbaseJsVersion,
              sessionId,
              size: msg.length || msg.byteLength,
            }
            ...
            if (action === "SignOut") {
              ...
              (action === "UpdateUser" || action === "GetPasswordSalts") &&
              (conn.keyValidated || changePassword)
            ) {
              ...
              switch (action) {
                ...
                case "GetPasswordSalts": {
                  response = await userController.getPasswordSaltsByUserId(
                    userId
                  )
                  break
                }
              ...
              switch (action) {
                ...
                case "GetDatabases": {
                  response = await db.getDatabases(
                    logChildObject,
                ...
                case "Bundle": {
                  ...
                  response = responseBuilder.errorResponse(
                    statusCodes["Gone"],
                    "Deprecated. Update to latest version of userbase-js."
                  )
                  break
                }
                case "InitBundleUpload": {
                  if (clientCanReadAndWriteBundleChunks(userbaseJsVersion)) {
                    ...
                    response = responseBuilder.errorResponse(
                      statusCodes["Gone"],
                      "Update to latest version of userbase-js to bundle."
                    )
                  }
                  break
                }
                ...
                case "GenerateFileId": {
                  response = await db.generateFileId(
                    logChildObject,
                    userId,
                    connectionId,
                    params.dbId
                ...
                case "PurchaseSubscription": {
                  response =
                    await userController.createSubscriptionPaymentSession(
                      logChildObject,
                      res.locals.app,
                      res.locals.admin,
                      res.locals.user,
                ...
                case "AuthenticateShareToken": {
                  response = await db.authenticateShareToken(
                    logChildObject,
                    res.locals.user,
                    params.shareTokenId
                  )
    ...
    wss.on("forgot-password", async (ws, req) => {
      ...
      const initForgotPassword = await userController.initForgotPassword(
        logChildObject,
        appId,
        username,
        deleteEndToEndEncryptedData
            ...
            ws.send(
              JSON.stringify({
                ...
                ecdsaKeyEncryptionKeySalt:
                  user["ecdsa-key-encryption-key-salt"],
          ...
          ws.on("message", async (msg) => {
            try {
              if (msg.length > FIVE_KB || msg.byteLength > FIVE_KB) {
                logger
                  .child({ ...logChildObject, size: msg.length })
                  .warn("Received large message over forgot-password")
                return ws.send("Message is too large")
              }
              ...
              logger
                .child({ ...logChildObject, err: e, msg })
                .error("Error in forgot-password Websocket")
            }
          })
        }
      }
    ...
    v1Api.post(
      "/auth/sign-in-with-session",
      userController.authenticateUser,
      userController.extendSession
    )
    v1Api.get("/auth/server-public-key", userController.getServerPublicKey)
    v1Api.get(
      "/auth/get-password-salts",
      userController.getPasswordSaltsController
    )
    ...
    v1Admin.post(
      ...
      bodyParser.raw({ type: "application/json" }),
      (req, res) =>
        stripe.handleWebhook(req, res, stripe.WEBHOOK_OPTIONS["TEST_CONNECT"])
    )
    v1Admin.post(
      "/stripe/prod/connect/webhook",
      bodyParser.raw({ type: "application/json" }),
      (req, res) =>
        stripe.handleWebhook(req, res, stripe.WEBHOOK_OPTIONS["PROD_CONNECT"])
    )
    ...
    v1Admin.post(
      "/list-app-users",
      (req, res, next) =>
        admin.authenticateAdmin(req, res, next, { authenticateAppName: true }),
      appController.listAppUsers
    )
    v1Admin.post(
      "/delete-app",
      admin.authenticateAdmin,
    ...
    v1Admin.post(
      ...
      admin.authenticateAdmin,
      appController.permanentDeleteAppController
    )
    ...
    v1Admin.delete(
      "/access-token",
      admin.authenticateAdmin,
      admin.deleteAccessToken
    )
    v1Admin.get("/account", admin.authenticateAdmin, admin.getAdminAccount)
    v1Admin.post(
      "/apps/:appId/encryption-mode",
      admin.authenticateAdmin,
      appController.modifyEncryptionMode
    )
    v1Admin.post(
      "/apps/:appId/domain",
      (req, res, next) =>
        admin.authenticateAdmin(req, res, next, { authenticateAppId: true }),
      appController.addDomainToWhitelist
    )
    v1Admin.get(
      "/apps/:appName/domains",
      (req, res, next) =>
        admin.authenticateAdmin(req, res, next, { authenticateAppName: true }),
      appController.getDomainWhitelist
    )
    v1Admin.delete(
      "/apps/:appId/domain",
      (req, res, next) =>
        admin.authenticateAdmin(req, res, next, { authenticateAppId: true }),
      appController.deleteDomainFromWhitelist
    )

    // endpoints for admin to manage their own account's payments to Userbase
    v1Admin.post(
      "/stripe/create-saas-payment-session",
      admin.authenticateAdmin,
      admin.createSaasPaymentSession
    )
    v1Admin.post(
      "/stripe/update-saas-payment-session",
      admin.authenticateAdmin,
      admin.updateSubscriptionPaymentSession
    )
    v1Admin.post(
      "/stripe/cancel-saas-subscription",
      admin.authenticateAdmin,
      admin.cancelSaasSubscription
    )
    v1Admin.post(
      "/stripe/resume-saas-subscription",
      admin.authenticateAdmin,
      admin.resumeSaasSubscription
    )
    v1Admin.post(
      "/stripe/storage-plan",
      admin.authenticateAdmin,
      admin.subscribeToStoragePlan
    )
    v1Admin.post(
      "/stripe/cancel-storage-subscription",
      admin.authenticateAdmin,
      admin.cancelStorageSubscription
    )
    v1Admin.post(
      "/stripe/resume-storage-subscription",
      admin.authenticateAdmin,
      admin.resumeStorageSubscription
    )

    // endpoints for admin to use payment portal to accept payments from their users
    v1Admin.post(
      "/stripe/connection/:authorizationCode",
      admin.authenticateAdmin,
      admin.completeStripeConnection
    )
    v1Admin.delete(
      "/stripe/connection",
      admin.authenticateAdmin,
      admin.disconnectStripeAccount
    )
    v1Admin.post(
      "/stripe/connected/apps/:appId/trial-period",
      admin.authenticateAdmin,
      appController.setTrialPeriod
    )
    v1Admin.delete(
      "/stripe/connected/apps/:appId/trial-period",
      admin.authenticateAdmin,
      appController.deleteTrial
    )
    v1Admin.post(
      "/stripe/connected/apps/:appId/enable-test-payments",
      admin.authenticateAdmin,
      appController.enableTestPayments
    )
    v1Admin.post(
      "/stripe/connected/apps/:appId/enable-prod-payments",
      admin.authenticateAdmin,
      appController.enableProdPayments
    )
    v1Admin.post(
      "/stripe/connected/apps/:appId/payment-required",
      admin.authenticateAdmin,
      appController.setPaymentRequired
    )

    // Access token endpoints
    v1Admin.post(
      "/users/:userId",
      admin.authenticateAccessToken,
      userController.updateProtectedProfile
    )
    v1Admin.get(
      "/users/:userId",
      admin.authenticateAccessToken,
      userController.adminGetUserController
    )
    v1Admin.get(
      "/apps/:appId",
      admin.authenticateAccessToken,
    ...
    v1Admin.get(
      "/apps/:appId/users",
      admin.authenticateAccessToken,
      appController.listUsersWithPagination
    )
    v1Admin.get(
      "/databases/:databaseId/users",
      admin.authenticateAccessToken,
      db.listUsersForDatabaseWithPagination
    )
    v1Admin.get(
      "/apps",
      admin.authenticateAccessToken,
      appController.listAppsWithPagination
    )
    v1Admin.get(
    ...
    http
      .createServer(internalServer)
      .listen(internalServerPort, () =>
        logger.info(
          `Internal server listening on http port ${internalServerPort}....`
        )
      )
    ...
    internalServer.post("/internal/notify-transaction", (req, res) => {
      ...
      try {
        logChildObject = {
          userId,
          connectionId,
          databaseId: transaction["database-id"],
          seqNo: transaction["seq-no"],
          req: trimReq(req),
        }
        logger
          .child(logChildObject)
...
function createAdmin({
  ...
  adminId,
  receiveEmailUpdates,
  storePasswordInSecretsManager = false,
}) {
