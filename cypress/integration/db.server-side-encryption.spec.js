    ...
    describe('Success Tests', function () {
      ...
      it('Get 10 Databases', async function () {
        ...
        for (let i = 0; i < numDatabases; i++) {
          ...
          expect(database, 'database keys').to.have.keys(['databaseName', 'databaseId', 'isOwner', 'readOnly', 'resharingAllowed', 'users', 'encryptionMode'])
          const { isOwner, readOnly, resharingAllowed, users, encryptionMode } = database
